...
export default function ItemCard({
  ...
  return (
    <div className="bg-surface rounded-3xl border border-separator/60 p-4 transition-all duration-200 hover:border-primary/15 hover:shadow-[0_4px_20px_rgba(0,0,0,0.06)] group">
      ...
      <div className="flex items-center gap-3 mt-3 pt-3 border-t border-separator/40">
        {item.url && (
          <a
            href={item.url}
            target="_blank"
      ...
      {item.note && (
        <p className="text-xs text-text-muted mt-2 line-clamp-1">{item.note}</p>
      )}
    </div>
  );
}
